...
impl PartialDecode {
    pub fn new(
        ...
        let rest = plain_header.payload_len().and_then(|len| {
            let packet_len = (buf.position() + len) as usize;
            if buf.get_ref().len() > packet_len {
                Some(buf.get_mut().split_off(packet_len))
            } else {
                None
            }
        });
